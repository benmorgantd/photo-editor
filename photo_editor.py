...
This script processes individual images or entire directories of JPEG, HEIC,
and camera RAW formats using adjustments defined in a JSON preset file. It handles
adjustments like HDR-to-SDR filmic tone mapping, exposure, global contrast, global
vibrance, global saturation, highlights, shadows, clarity, texture, customizable
grain structures, and selective color adjustments before exporting Instagram-optimized JPEGs.
"""
...
# Supported file extensions for batch processing (including common camera RAW formats)
...
class PhotoEditor:
    ...
    def __init__(self, image_path: str):
        ...
        Args:
            image_path: Path to the input image or RAW file.
        """
        self.image_path = image_path
        self.original_image = self._load_image()
        
    def _load_image(self) -> np.ndarray:
        """Loads HEIC, JPEG, or RAW images and converts them to a float32 RGB array.

        This method branches depending on the file extension. For raw files, it
        uses rawpy to extract a high-fidelity 16-bit array. For standard images, it
        accounts for smartphone EXIF orientation metadata using PIL.
        ...
        # Handle Camera RAW files
        if ext in RAW_EXTENSIONS:
            with rawpy.imread(self.image_path) as raw:
                # Postprocess options:
                # - use_camera_wb=True: Pulls the white balance set by the camera body
                # - half_size=False: Forces full-resolution demosaicing
                # - no_auto_bright=True: Disables linear stretching to preserve sensor clipping points
                # - output_bps=16: Returns a high-bit-depth uint16 array [0-65535]
            ...
            # Normalize 16-bit data into our 32-bit float workspace
            return rgb_16.astype(np.float32) / 65535.0

        # Handle Standard JPEG/HEIC image containers
        else:
...
if __name__ == "__main__":
    main()"""A headless photo editing utility mimicking basic Adobe Lightroom functionality.
